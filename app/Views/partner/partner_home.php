                    ...
                    <table class="table text-center bg-primary-hover tr-rounded order-tbl mt-2">
                        ...
                        <tbody>
                            <?php foreach ($trending_products as $trending): ?>
                                <tr>
                                    <td class="text-start"><?= esc($trending['ProductName']) ?></td>
                                    <td class="text-end"><?= esc($trending['rating']) ?></td>
                                </tr>
                            <?php endforeach; ?>
                        </tbody>
                    </table>
								...
								<div class="card transaction-table">
									<div class="card-header border-0 flex-wrap pb-0">
										<div class="mb-2">
											...
											<p class="mb-sm-3 mb-0">View your recent transactions</p>
										</div>
									...
									<div class="card-body p-0">
										<div class="tab-content" id="myTabContent1">
											...
											foreach ($periods as $period):
												$isActive = $period === 'Week' ? 'show active' : '';
											?>
											<div class="tab-pane fade <?= $isActive ?>" id="<?= $period ?>" role="tabpanel" aria-labelledby="<?= $period ?>-tab">
												<div class="table-responsive">
													<table class="table table-responsive-md">
														...
														<tbody>
															...
															$filteredTransactions = array_filter($transactions, function($transaction) use ($period) {
																...
																switch($period) {
																	case 'Week':
																		return $transactionDate > $now->modify('-1 week');
																	case 'month':
																		return $transactionDate > $now->modify('-1 month');
																	case 'year':
																		return $transactionDate > $now->modify('-1 year');
																}
															});
